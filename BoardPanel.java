package manjava;

import java.awt.*;
import java.awt.event.*;
import java.awt.geom.*;
import javax.swing.*;

public class BoardPanel extends JPanel {

    private Board board;
    private MancalaController controller;
    private StyleStrategy style = new ConcreteStyleStrategy2();
    
    private Shape[] slots = new Shape[Board.TOTAL_SLOTS];
    private RoundRectangle2D boardShape;

    public BoardPanel() {
        setPreferredSize(new Dimension(825, 325));
        addMouseListener(new MouseAdapter() {
            @Override
            public void mousePressed(MouseEvent e) {
                int clickedIntex = getClickedSlot(e.getPoint());
                if (clickedIntex != -1  && clickedIntex != Board.MANCALA_A && clickedIntex != Board.MANCALA_B && controller != null) {                 
                    controller.handlePitSelection(clickedIntex);
                }
            }
        });
    }

    void setBoard(Board board) {
        this.board = board;
        repaint();
    }

    public void setController(MancalaController controller) {
        this.controller = controller;
    }

    @Override
    protected void paintComponent(Graphics g) {
        super.paintComponent(g);
        Graphics2D g2 = (Graphics2D) g;
        createShapes();
        if(style != null) {
            style.drawBoardBackground(g2, boardShape);
        } else {
            g2.setColor(new Color(180, 180, 180));
            g2.fill(boardShape);
            g2.setColor(Color.BLACK);
            g2.setStroke(new BasicStroke(3));
            g2.draw(boardShape);
        }

        for (int i = 0; i < slots.length; i++) {
            if (slots[i] == null) continue;

            if (i == Board.MANCALA_A || i == Board.MANCALA_B) {
                drawMancala(g2, i);
            } else {
                drawPit(g2, i);
            }
        }
    }

    private void createShapes() {
        boardShape = new RoundRectangle2D.Double(2, 2, 825, 325, 50, 50);

        int pitSize = 75;
        int gap = 25;
        int startX = 128;

        int topY = 65;
        int bottomY = 185;

        slots[Board.MANCALA_B] = new RoundRectangle2D.Double(25, 55, 85, 215, 45, 45);
        slots[Board.MANCALA_A] = new RoundRectangle2D.Double(720, 55, 85, 215, 45, 45);

        for (int i = 0; i < 6; i++) {
            slots[i] = new Ellipse2D.Double(startX + i * (pitSize + gap), bottomY, pitSize, pitSize);
            slots[12 - i] = new Ellipse2D.Double(startX + i * (pitSize + gap), topY, pitSize, pitSize);
        }
    }

    private void drawPit(Graphics2D g2, int index) {
        Shape pit = slots[index];
        Rectangle bounds = pit.getBounds();
        if(style != null) {
            style.drawPit(g2, pit, false);
        } else {
            g2.setColor(Color.LIGHT_GRAY);
            g2.fill(pit);
            g2.setColor(Color.WHITE);
            g2.draw(pit);
        }
        String label = getPitLabel(index);
        g2.setColor(Color.BLACK);
        g2.drawString(label, bounds.x + 25, bounds.y - 8);
        int stones = 0;
        if(board != null) {
            stones = board.getStonesIn(index);
        }
        drawStones(g2, bounds, stones);
    }

    private void drawMancala(Graphics2D g2, int index) {
        Shape mancala = slots[index];
        Rectangle bounds = mancala.getBounds();
        if(style != null) {
            style.drawMancala(g2, mancala);
        } else {
            g2.setColor(Color.LIGHT_GRAY);
            g2.fill(mancala);
            g2.setColor(Color.BLACK);
            g2.draw(mancala);
        }
        String label = index == Board.MANCALA_A ? "A" : "B";
        g2.setColor(Color.BLACK);
        g2.drawString(label, bounds.x + bounds.width / 2 - 4, bounds.y - 8);
        int stones = 0;
        if(board != null) {
            stones = index == Board.MANCALA_A ? board.getMancala(0).getStones() : board.getMancala(1).getStones();
        }
        drawStones(g2, bounds, stones);
    }

    private void drawStones(Graphics2D g2, Rectangle area, int stones) {
        for(int i = 0; i < stones; i++) {
            int x = area.x + 15 + (i % 4) * 14;
            int y = area.y + 18 + (i / 4) * 14;
            if(style != null) {
                style.drawStone(g2,x,y );
            } else {
            	g2.setColor(Color.WHITE);
                g2.fillOval(x, y, 10, 10);
            }
        }
    }

    private int getClickedSlot(Point p) {
        for (int i = 0; i < slots.length; i++) {
            if (slots[i] != null && slots[i].contains(p)) {
                return i;
            }
        }
        return -1;
    }

    private String getPitLabel(int index) {
        if (index >= 0 && index <= 5) {
            return "A" + (index + 1);
        }
        return "B" + (13 - index);
    }

    public void setStyle(StyleStrategy style) {
        this.style = style;
        repaint();
    }
}